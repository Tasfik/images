...
vector <ll> DhakaRoad[NodeMX], g[NodeMX], cost[NodeMX], type[NodeMX];
map <PII, ll> road;
PII RoadList[NodeMX];
ll dist[NodeMX], path[NodeMX], pathType[NodeMX];

void Dijkstra(ll source) {
	map<ll, ll> m;
	
	for (ll i = 1; i < DhakaRoadMX; i++)
	{
		dist[i] = INT_MAX;
		path[i] = -1;
		pathType[i] = -1;
